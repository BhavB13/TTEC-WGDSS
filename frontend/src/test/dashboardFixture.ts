import type { DashboardSnapshot } from "../types/dashboard";

export const dashboardFixture: DashboardSnapshot = {
  snapshot_id: "snapshot-capacity-plan-1",
  weather: {
    timestamp: "2026-06-27T12:00:00-04:00",
    temperature_c: 30,
    humidity_percent: 75,
    rainfall_mm_hr: 0.5,
    cloud_cover_percent: 45,
    wind_speed_kmh: 14,
    weather_condition: "Partly cloudy",
    heat_index_c: 34,
    rain_severity: "LIGHT",
    provider_name: "Open-Meteo",
  },
  grid: {
    timestamp: "2026-06-27T12:00:00-04:00",
    current_demand_mw: 950,
    current_generation_mw: 960,
    total_available_capacity_mw: 1200,
    reserve_margin_percent: 26.3,
    spinning_reserve_mw: 80,
    spinning_reserve_source: "GSYS SYSTEM_CORRECTED_SPIN_TOTAL",
    grid_status: "NORMAL",
    demand_period: "AFTERNOON",
    source_provider: "MockGridProvider",
    generation_units: [],
  },
  forecast: {
    items: Array.from({ length: 6 }, (_, index) => ({
      forecast_timestamp: `2026-06-27T${13 + index}:00:00-04:00`,
      temperature_c: 30 + index / 10,
      humidity_percent: 74,
      rainfall_mm_hr: 0.2,
      cloud_cover_percent: 50,
      wind_speed_kmh: 15,
      weather_condition: "Partly cloudy",
      heat_index_c: 34,
      precipitation_probability_percent: 20,
      confidence_score: 0.9,
      rain_severity: "LIGHT",
      provider_name: "Consensus (Open-Meteo + MET Norway)",
      source_count: 3,
      source_names: ["Open-Meteo Best Match", "MET Norway", "Open-Meteo NOAA GFS"],
      temperature_spread_c: 0.4,
      cloud_cover_spread_percent: 12,
    })),
  },
  probability: {
    probability_score: 0.5,
    capacity_risk_percent: 50,
    risk_level: "HIGH",
    capacity_status: "Prepare Generation",
    forecast_demand_30m: 920,
    forecast_demand_60m: 930,
    forecast_demand_mw: 930,
    forecast_uncertainty_mw: 22,
    forecast_tra_mw: 960,
    projected_reserve_mw: 30,
    reserve_surplus_mw: 0,
    reserve_deficit_mw: 0,
    reserve_insufficient_horizon_minutes: 120,
    reserve_insufficient_at: "2026-06-27T14:00:00-04:00",
    uncertainty_source: "CALIBRATED_HISTORICAL_RESIDUALS",
    tra_projection_basis: "CURRENT_TRA_HELD_SCENARIO_NO_DISPATCH_PLAN",
    factors: ["Conditions remain stable"],
    reason: "Conditions remain stable",
    expected_shortfall_mw: 4.2,
    projected_shortfall_mw: 18.5,
    recommended_capacity_mw: 0,
    decision_confidence: 0.86,
    decision_action: "MONITOR",
    severity_level: "FAST_START",
    urgency: "LEAD_TIME_AVAILABLE",
    peak_risk_horizon_minutes: 120,
    peak_risk_timestamp: "2026-06-27T14:00:00-04:00",
    decision_deadline_minutes: 60,
    decision_deadline_at: "2026-06-27T13:00:00-04:00",
    formula_version: "wgdss-capacity-risk-v5",
    drivers: [
      {
        label: "Forecast uncertainty can reduce reserve below the 30 MW target",
        direction: "INCREASES_RISK",
        category: "UNCERTAINTY",
      },
      {
        label: "Forecast weather reduces demand by 8.0 MW",
        direction: "REDUCES_RISK",
        category: "WEATHER",
      },
      {
        label: "Operating policy is prototype unconfirmed",
        direction: "QUALITY_WARNING",
        category: "POLICY",
      },
    ],
    risk_profile: [
      {
        horizon_minutes: 60,
        forecast_timestamp: "2026-06-27T13:00:00-04:00",
        probability: 0.1335,
        forecast_demand_mw: 910,
        forecast_uncertainty_mw: 18,
        forecast_lower_mw: 880.4,
        forecast_upper_mw: 939.6,
        confidence_level: 0.9,
        immediate_online_capacity_mw: 960,
        safe_online_capacity_mw: 930,
        required_reserve_mw: 30,
        online_headroom_mw: 50,
        reserve_adjusted_headroom_mw: 20,
        expected_shortfall_mw: 1.2,
        conservative_shortfall_mw: 9.6,
        expected_load_rise_mw: 0,
        weather_effect_mw: -8,
        forecast_confidence: 0.86,
        startup_lead_time_minutes: 60,
        decision_deadline_minutes: 0,
        decision_deadline_at: "2026-06-27T12:00:00-04:00",
        urgency: "ACTION_DUE",
        capacity_risk_percent: 13.35,
        forecast_tra_mw: 960,
        projected_reserve_mw: 50,
        reserve_surplus_mw: 20,
        reserve_deficit_mw: 0,
        capacity_status: "Normal",
        reserve_expected_insufficient: false,
        uncertainty_source: "CALIBRATED_HISTORICAL_RESIDUALS",
        tra_projection_basis: "CURRENT_TRA_HELD_SCENARIO_NO_DISPATCH_PLAN",
      },
      {
        horizon_minutes: 120,
        forecast_timestamp: "2026-06-27T14:00:00-04:00",
        probability: 0.5,
        forecast_demand_mw: 930,
        forecast_uncertainty_mw: 22,
        forecast_lower_mw: 893.8,
        forecast_upper_mw: 966.2,
        confidence_level: 0.9,
        immediate_online_capacity_mw: 960,
        safe_online_capacity_mw: 930,
        required_reserve_mw: 30,
        online_headroom_mw: 30,
        reserve_adjusted_headroom_mw: 0,
        expected_shortfall_mw: 8.8,
        conservative_shortfall_mw: 36.2,
        expected_load_rise_mw: 0,
        weather_effect_mw: -5,
        forecast_confidence: 0.82,
        startup_lead_time_minutes: 60,
        decision_deadline_minutes: 60,
        decision_deadline_at: "2026-06-27T13:00:00-04:00",
        urgency: "LEAD_TIME_AVAILABLE",
        capacity_risk_percent: 50,
        forecast_tra_mw: 960,
        projected_reserve_mw: 30,
        reserve_surplus_mw: 0,
        reserve_deficit_mw: 0,
        capacity_status: "Prepare Generation",
        reserve_expected_insufficient: true,
        uncertainty_source: "CALIBRATED_HISTORICAL_RESIDUALS",
        tra_projection_basis: "CURRENT_TRA_HELD_SCENARIO_NO_DISPATCH_PLAN",
      },
    ],
  },
  recommendation: {
    probability_score: 0.5,
    capacity_risk_percent: 50,
    risk_level: "HIGH",
    capacity_status: "Prepare Generation",
    forecast_demand_30m: 920,
    forecast_demand_60m: 930,
    forecast_demand_mw: 930,
    forecast_uncertainty_mw: 22,
    forecast_tra_mw: 960,
    projected_reserve_mw: 30,
    reserve_surplus_mw: 0,
    reserve_deficit_mw: 0,
    reserve_insufficient_horizon_minutes: 120,
    reserve_insufficient_at: "2026-06-27T14:00:00-04:00",
    uncertainty_source: "CALIBRATED_HISTORICAL_RESIDUALS",
    tra_projection_basis: "CURRENT_TRA_HELD_SCENARIO_NO_DISPATCH_PLAN",
    factors: ["Conditions remain stable"],
    reason: "Conditions remain stable",
    recommendation: "PREPARE ADDITIONAL GENERATION",
    decision_action: "REVIEW CAPACITY START PLAN",
    generator_set: "AGGREGATE START BLOCK PLAN",
    recommended_capacity_mw: 30,
    projected_shortfall_mw: 0,
    expected_shortfall_mw: 0,
    expected_load_rise_mw: 35,
    expected_rise_minutes: 60,
    startup_time_minutes: 0,
    decision_confidence: 0.9,
    weather_effect_mw: 8,
  },
  capacity_plan: {
    snapshot_id: "snapshot-capacity-plan-1",
    status: "AVAILABLE",
    action_source: "SYSTEM_RECOMMENDED",
    advisory_only: true,
    advisory_notice: "ADVISORY ONLY - MANUAL OPERATOR ACTION REQUIRED",
    system_suggestion:
      "REVIEW START OF 2 X SMALL FAST-START SET (30.0 MW TOTAL)",
    system_suggestion_basis: [
      "No-action peak risk is 50.0% against the 20.0% Watch threshold",
      "Peak modeled demand is 930.0 MW at +120 minutes with current TRA held at 960.0 MW",
      "The selected configured plan is 2 x Small fast-start set, adding 30.0 MW after its lead time",
      "Modeled post-plan peak risk is 13.4% with a maximum startup lead of 20 minutes",
    ],
    issue_time: "2026-06-27T12:00:00-04:00",
    current_tra_mw: 960,
    current_tra_observed_at: "2026-06-27T12:00:00-04:00",
    current_tra_age_seconds: 0,
    current_tra_source: "MockGridProvider aggregate online capacity",
    current_tra_quality_status: "GOOD",
    current_tra_projection_basis: "CURRENT_TRA_HELD_NO_ACTION",
    required_reserve_mw: 30,
    target_risk_probability: 0.2,
    baseline_peak_risk_percent: 50,
    post_plan_peak_risk_percent: 13.35,
    risk_reduction_percentage_points: 36.65,
    first_unprotected_horizon_minutes: null,
    first_unprotected_at: null,
    interim_unmitigated_risk: false,
    unresolved_capacity_mw: 0,
    block_definitions: [
      {
        block_id: "small-fast-start",
        label: "Small fast-start set",
        block_class: "SMALL",
        unit_capacity_mw: 15,
        startable_count: 3,
        startup_lead_time_minutes: 20,
        enabled: true,
        provenance: "CONFIGURABLE_PROTOTYPE_ROSTER",
        verification_status: "UNCONFIRMED",
      },
      {
        block_id: "heavy-start-unconfigured",
        label: "Heavy start capacity",
        block_class: "HEAVY",
        unit_capacity_mw: null,
        startable_count: 0,
        startup_lead_time_minutes: 60,
        enabled: false,
        provenance: "CAPACITY_NOT_CONFIGURED",
        verification_status: "UNCONFIGURED",
      },
    ],
    recommended_actions: [
      {
        block_id: "small-fast-start",
        block_label: "Small fast-start set",
        block_class: "SMALL",
        count: 2,
        unit_capacity_mw: 15,
        total_capacity_mw: 30,
        startup_lead_time_minutes: 20,
        start_at: "2026-06-27T13:40:00-04:00",
        start_by: "2026-06-27T13:40:00-04:00",
        expected_online_at: "2026-06-27T14:00:00-04:00",
        verification_status: "UNCONFIRMED",
        action_status: "PROPOSED",
        applied_to_projection: true,
      },
    ],
    evaluated_actions: [
      {
        block_id: "small-fast-start",
        block_label: "Small fast-start set",
        block_class: "SMALL",
        count: 2,
        unit_capacity_mw: 15,
        total_capacity_mw: 30,
        startup_lead_time_minutes: 20,
        start_at: "2026-06-27T13:40:00-04:00",
        start_by: "2026-06-27T13:40:00-04:00",
        expected_online_at: "2026-06-27T14:00:00-04:00",
        verification_status: "UNCONFIRMED",
        action_status: "PROPOSED",
        applied_to_projection: true,
      },
    ],
    profile: [
      {
        horizon_minutes: 60,
        forecast_timestamp: "2026-06-27T13:00:00-04:00",
        forecast_demand_mw: 910,
        forecast_uncertainty_mw: 18,
        baseline_tra_mw: 960,
        baseline_reserve_mw: 50,
        baseline_capacity_risk_percent: 13.35,
        baseline_capacity_status: "Normal",
        planned_tra_mw: 960,
        applied_start_capacity_mw: 0,
        planned_reserve_mw: 50,
        planned_reserve_surplus_mw: 20,
        planned_reserve_deficit_mw: 0,
        planned_capacity_risk_percent: 13.35,
        planned_capacity_status: "Normal",
        required_reserve_mw: 30,
      },
      {
        horizon_minutes: 120,
        forecast_timestamp: "2026-06-27T14:00:00-04:00",
        forecast_demand_mw: 930,
        forecast_uncertainty_mw: 22,
        baseline_tra_mw: 960,
        baseline_reserve_mw: 30,
        baseline_capacity_risk_percent: 50,
        baseline_capacity_status: "Prepare Generation",
        planned_tra_mw: 990,
        applied_start_capacity_mw: 30,
        planned_reserve_mw: 60,
        planned_reserve_surplus_mw: 30,
        planned_reserve_deficit_mw: 0,
        planned_capacity_risk_percent: 8.65,
        planned_capacity_status: "Normal",
        required_reserve_mw: 30,
      },
    ],
    configuration_status: "PROTOTYPE_UNCONFIRMED",
    warnings: [
      "Current TRA is simulated; this plan is for replay or training only",
      "Heavy-set capacity is unconfigured; no MW-specific heavy start is proposed",
      "One or more proposed block capacities are unconfirmed and must be verified",
    ],
  },
  calibration: {
    selected_scenario_key: "typical",
    selected_scenario_label: "Typical Day",
    selected_temperature_c: 30,
    selection_reason: "Typical profile is the closest match",
    selection_confidence: 0.8,
    scenario_scores: { hot: 0.5, typical: 0.8, rainy: 0.3 },
    scenarios: [],
  },
  data_quality: {
    overall_status: "GOOD",
    weather_status: "LIVE",
    grid_status: "SIMULATED",
    calibration_status: "CALIBRATED",
    weather_source: "Open-Meteo",
    grid_source: "MockGridProvider",
    observed_at: "2026-06-27T12:00:00-04:00",
    age_seconds: 30,
    is_stale: false,
    fallback_used: false,
    notes: [],
  },
  demand_forecast: {
    horizons: [
      {
        horizon_hours: 1,
        forecast_timestamp: "2026-06-27T13:00:00-04:00",
        forecast_demand_mw: 990,
        forecast_uncertainty_mw: 18,
        model_name: "persistence",
        model_version: "demand-forecast-v1.3",
        baseline_name: "persistence",
        baseline_forecast_mw: 985,
        quality_status: "BASELINE_ACTIVE",
      },
    ],
  },
  model_status: {
    active_model: "persistence",
    model_version: "demand-forecast-v1.3",
    mode: "BASELINE_ACTIVE",
    trained_through: "2026-06-27T13:00:00-04:00",
    metrics: {
      mae: 10,
      rmse: 12,
      mape: 1.2,
      residual_std: 18,
    },
    baseline_comparison: {
      best_baseline: "persistence",
      ml_beats_baseline: false,
    },
  },
  scada_status: {
    mode: "historical_replay",
    source: "scada.csv",
    latest_snapshot: "2026-06-27T12:00:00-04:00",
    quality_status: "GOOD",
    missing_fields: "",
  },
};

export const replayDashboardFixture: DashboardSnapshot = {
  ...dashboardFixture,
  weather: {
    ...dashboardFixture.weather,
    timestamp: "2025-06-01T08:00:00-04:00",
    provider_name: "Simulation replay · historical weather",
  },
  grid: {
    ...dashboardFixture.grid,
    timestamp: "2025-06-01T08:00:00-04:00",
    source_provider: "SyntheticReplayProvider",
  },
  forecast: {
    items: dashboardFixture.forecast.items.map((period, index) => ({
      ...period,
      forecast_timestamp: `2025-06-01T${String(9 + index).padStart(2, "0")}:00:00-04:00`,
    })),
  },
  replay: {
    status: {
      mode: "simulation",
      dataset_label: "2025 hourly SCADA + weather demonstration",
      dataset_start: "2025-01-01T00:00:00-04:00",
      dataset_end: "2025-12-31T23:00:00-04:00",
      replay_start: "2025-06-01T00:00:00-04:00",
      replay_end: "2025-06-30T23:00:00-04:00",
      cursor_at: "2025-06-01T08:00:00-04:00",
      is_playing: false,
      step_minutes: 60,
      speed_multiplier: 600,
      progress_percent: 1.1,
      revealed_records: 9,
      total_replay_records: 720,
      source: "WGDSS 12-Month Synthetic SCADA/Weather Demonstration",
      clock_aligned: true,
    },
    operational_history: [],
    full_day_load_forecast: Array.from({ length: 24 }, (_, hour) => ({
      timestamp: `2025-06-01T${String(hour).padStart(2, "0")}:00:00-04:00`,
      forecast_demand_mw: 800 + hour * 10,
      historical_average_mw: 790 + hour * 9,
      actual_demand_mw: hour <= 8 ? 795 + hour * 10 : null,
      actual_temperature_c: hour <= 8 ? 25 + hour * 0.4 : null,
      forecast_temperature_c: hour > 8 ? 28.6 + (hour - 9) * 0.15 : null,
      uncertainty_mw: 20,
      weather_impact_mw: 8,
      weather_confidence: 0.9,
      weather_source_count: 3,
    })),
    monthly_history: Array.from({ length: 12 }, (_, index) => ({
      month: new Intl.DateTimeFormat("en", { month: "short", timeZone: "UTC" }).format(new Date(Date.UTC(2025, index, 1))),
      average_demand_mw: 880 + index * 4,
      peak_demand_mw: 1220 + index * 5,
      average_temperature_c: 27 + index / 10,
      rainfall_total_mm: 40 + index * 8,
      data_phase: index === 5 ? "REPLAY_SOURCE" as const : "HISTORICAL" as const,
    })),
    summary: {
      historical_months: 11,
      historical_record_count: 8040,
      historical_average_demand_mw: 910,
      historical_peak_demand_mw: 1320,
      replay_month_label: "June 2025",
      current_day_peak_forecast_mw: 1030,
      forecast_model: "WeatherFeatureRidge",
      forecast_mode: "ML_ACTIVE",
      forecast_mae_mw: 12,
      baseline_mae_mw: 20,
      residual_std_mw: 15,
      training_rows: 3600,
      weather_features: ["temperature_c", "humidity_percent", "rainfall_mm_hr"],
    },
  },
};
